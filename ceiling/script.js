...
async function loadData() {
  try {
    const [products, filenames, scenarioInfo, moduleInfo] = await Promise.all([
...
function parseWeight(value) {
  // Extracts the leading number from "7.16 (1000×500)" → 7.16
...
function uniqueSortedWeights(values) {
  return [...new Set(values.filter(Boolean))].sort((a, b) => {
    const na = parseWeight(a);
    const nb = parseWeight(b);
...
function onGewichtChange() {
  ...
  console.log("Selected weight:", dom.gewichtSelect.value);   // ← ADD
...
function renderProductOverview(record) {
  ...
  dom.selectedProductVariant.textContent = `${family || "—"} · ${gewicht || "—"}`;
...
function renderScenarios(scenarios) {
  const record = findSelectedProductRecord();
  const productType = record ? field(record, "Produkttyp") : "";
...
function updateSelectionSummary() {
  ...
  dom.selectionSummary.textContent = `Loaded configuration: ${family} · ${productType} · ${gewicht}`;
}
...
function renderModuleBarChart(record, selectedScenarios) {
  ...
  // Value accessor for Lindner side (ceiling format)
  const getVal = (key, s) => {
    if (key === "(A1-C4)") return getScenarioTotalA1C4(record, s.id);
    return getScenarioModuleValue(record, key, s.id);
  };
  ...
  if (state.chartView === "modules") {
    const allGroups = buildModuleChartGroups(record, selectedScenarios);
    ...
    if (state.chartDisplayMode === "percent") {
      ...
      chart.innerHTML = `
        ...
        ${_renderDonutRingsSavedBase(selectedScenarios,
          s => getScenarioTotalA1C4(record, s.id),
          s => getScenarioModuleValue(record, "D", s.id))}`;
    } else {
      const a1c4Group = {
        key: "A1-C4", label: "A1-C4", sub: "Total", tone: "total",
        bars: selectedScenarios.map((s, i) => ({ label: s.name, seriesIndex: i, value: getScenarioTotalA1C4(record, s.id) })).filter(b => b.value !== null)
      };
      const dGroup = {
        key: "D", label: "D", sub: "Benefit (Module D)", tone: "benefits",
        bars: selectedScenarios.map((s, i) => ({ label: s.name, seriesIndex: i, value: getScenarioModuleValue(record, "D", s.id) })).filter(b => b.value !== null)
      };
      const netGroup = {
        key: "Net", label: "Net Carbon", sub: "A1-C4 + D", tone: "total",
        bars: selectedScenarios.map((s, i) => {
          const a = getScenarioTotalA1C4(record, s.id);
          const d = getScenarioModuleValue(record, "D", s.id);
...
// A1-A3 — scenario-independent value, shown per scenario (two bars)
groups.push({
  key: "A1-A3", label: "A1-A3", sub: "Product stage", tone: "production",
  bars: selectedScenarios.map((s, i) => ({
    label: s.name,
    value: toNumber(field(record, "A1-A3")),   // ← scenario-1 column only, same for all
    seriesIndex: i
  })).filter(b => b.value !== null)
});
...
groups.push({
  ...
  bars: selectedScenarios.map((s, i) => ({
    label: s.name,
    value: toNumber(field(record, "A4")),
    seriesIndex: i
  })).filter(b => b.value !== null)
});
...
groups.push({
  ...
  bars: selectedScenarios.map((s, i) => ({
    label: s.name,
    value: toNumber(field(record, "A5")),
    seriesIndex: i
  })).filter(b => b.value !== null)
});

  // C1–C4 individual, scenario-dependent
  ["C1", "C2", "C3", "C4"].forEach(base => {
    groups.push({
      key: base, label: base, sub: eolMap[base], tone: "endlife",
      bars: selectedScenarios.map((s, i) => ({
        label: s.name, value: getScenarioModuleValue(record, base, s.id), seriesIndex: i
      })).filter(b => b.value !== null)
    });
  });
  ...
  groups.push({
    ...
    bars: selectedScenarios.map((s, i) => ({
      label: s.name, value: getScenarioModuleValue(record, "D", s.id), seriesIndex: i
    })).filter(b => b.value !== null)
  });
...
function getLegendDotStyle(index) {
  // Matches CSS series-0 (steel) and series-1 (red)
  if (index === 0) return "background: linear-gradient(180deg, #044459, #1A8FA8);";
  return "background: linear-gradient(180deg, #AE0C1E, #E40428);";
}
...
function initTheme() {
  // Light mode deferred system-wide — ceiling is dark-only for now.
  applyTheme("dark");
}
...
/* ════════════════════════════════════════════════════════════════════
   COMPETITOR PANEL — Metal Ceilings (B.6)
   Reads shared ../competitor/ files, filtered by division.
   ════════════════════════════════════════════════════════════════════ */
const CMP_DIVISION = "Metal Ceilings";

const cmpState = {
  impacts: [], products: [], epds: [], pdfMap: [],
  selectedScenarios: [], chartView: "modules", chartDisplayMode: "values",
  _loaded: false, impact: null, activeModules: null,
  scenarios: [],
...
function toggleCompetitorPanel(open) {
  ...
  // Action buttons: hide EPD/LCA downloads in competitor mode
...
/* ════════════════════════════════════════════════════════════════════
   COMPETITOR — B.6-B: Variant selection + result tiles
   ════════════════════════════════════════════════════════════════════ */
...
function onCmpVariantChange() {
  ...
  document.querySelectorAll("[data-cmp-view]").forEach(b =>
    b.classList.toggle("active", b.dataset.cmpView === "modules"));
  ...
  // Product image (from competitor_epd.csv 'image' column, in ../competitor/)
  ...
  // Enable Export PDF (wired in B.4); keep disabled for now
  if (dom.exportPdfBtn) {
    dom.exportPdfBtn.classList.remove("disabled");
    dom.exportPdfBtn.setAttribute("aria-disabled", "false");
  }
...
function cmpRenderScenarios(scenarios) {
  ...
  cmpDom.scenarioList.innerHTML = scenarios.map(s => {
    ...
    return `<div class="scenario-info-card ${active ? "selected" : ""}">
      <div class="scenario-title-row"><div class="scenario-name">${escapeHtml(s.name)}</div></div>
    </div>`;
  }).join("");
...
function cmpRenderResults(impact, scenarios) {
  ...
  // Chart rendering comes in B.6-D
...
function cmpValueLines(rows) {
  ...
  return rows.length ? rows.map(row => {
    ...
    return `<div class="compare-line">
      <div class="compare-meta"><span>${escapeHtml(row.name)}</span><strong>${formatMetric(row.value)}</strong></div>
...
/* ════════════════════════════════════════════════════════════════════
   B.6-C — BODEN CHART ENGINE (shared: Lindner + Competitor)
   Scenario palette, exploding pies, donut rings.
   ════════════════════════════════════════════════════════════════════ */

/* Scenario colors — ONE source of truth (matches CSS series-0/1) */
...
/* Generic pie renderer — takes a value accessor so it works for both sides */
...
function cmpRenderChart(impact, selected) {
  ...
  // Competitor value accessor (suffix format)
...
/* ─── Chart control handlers (both sides) ─── */
document.addEventListener("click", e => {
  ...
  if (viewBtn && viewBtn.dataset.view) {
    ...
    if (v !== state.chartView) {
      ...
      const record = findSelectedProductRecord();
      if (record) renderModuleBarChart(record, getAvailableScenarios().filter(s => state.selectedScenarios.includes(s.id)));
    }
    return;
  }
  ...
  if (cDispBtn) {
    ...
    if (m !== cmpState.chartDisplayMode) {
...
async function exportPdf() {
  ...
  const family = field(record, "Produktfamilie");
  const prodType = field(record, "Produkttyp");
  const gewicht = field(record, "Gewicht");
  ...
  await window.LindnerPDF.export({
    ...
    techItems: [
      ...
      ["Weight (kg/m\u00B2)", (gewicht || "--").replace(/×/g, "x")]
    ],
    selected,
    impact: record,
    // Ceiling accessors: A1-A3/A4/A5 scenario-independent; C1-C4/D per scenario
    getA1c4:   s => getScenarioTotalA1C4(record, s.id),
    getD:      s => getScenarioModuleValue(record, "D", s.id),
    getModule: (col, s) => {
      if (col === "A1-A3" || col === "A4" || col === "A5") return toNumber(field(record, col));
      return getScenarioModuleValue(record, col, s.id);
    },
    getFixed:  col => toNumber(field(record, col)),
    buildModGroups: () => buildModuleChartGroups(record, selected),
    getScenarioDesc: s => getScenarioInfo(prodType, s.name) || "",
    filename: `CarbonReport_${(prodType || "product").replace(/\s+/g, "_")}_${today.replace(/\s/g, "")}.pdf`
  });
}
/* ─── Competitor PDF (ceiling) ─── */
...
/* ════════════════════════════════════════════════════════════════════
   ARIA CONTEXT BUILDER — Metal Ceilings
   ════════════════════════════════════════════════════════════════════ */
function buildCeilingAriaContext() {
  const ctx = { page: 'Metal Ceilings (ceiling)', mode: 'Lindner Products' };

  // Competitor mode?
  const isCmp = document.getElementById('competitorTabBtn')?.getAttribute('aria-pressed') === 'true';
  if (isCmp && cmpState?.impact) {
  ...
  const record = findSelectedProductRecord();
  if (!record) {
    ctx.userState = 'No product selected yet. User needs to pick a product family, product type, and weight.';
    return ctx;
  }
  ...
  ctx.product = {
    family: field(record, 'Produktfamilie'),
    type: prodType,
    weight: field(record, 'Gewicht')
  };
  ctx.epd = { issueDate: '20.08.2025', validTo: '19.08.2030' };

  // A1-A3/A4/A5 scenario-independent; C1-C4/D per scenario
  ctx.fixedModules = {
    'A1-A3': field(record, 'A1-A3'),
    'A4': field(record, 'A4'),
    'A5': field(record, 'A5')
  };
  ctx.selectedScenarios = selected.map(s => {
    ...
    return {
      name: s.name,
      C1: getScenarioModuleValue(record, 'C1', s.id),
      C2: getScenarioModuleValue(record, 'C2', s.id),
      C3: getScenarioModuleValue(record, 'C3', s.id),
      C4: getScenarioModuleValue(record, 'C4', s.id),
      D: d,
      a1c4Total: a !== null ? a.toFixed(2) : null,
      netCarbon: (a !== null && d !== null) ? (a + d).toFixed(2) : null
    };
  });
  ...
  if (state.scenarioInfo?.length) {
    ...
    scenarios.forEach(s => {
      const desc = getScenarioInfo(prodType, s.name);
      if (desc) ctx.scenarioDescriptions[s.name] = desc;
    });
  }

  // Domain note (from EPD) so ARIA explains data correctly
  ctx.note = 'A1-A3, A4, A5 are scenario-independent (same value for both scenarios). C1-C4 and D differ per scenario. A1-A3 is the dominant stage (~19.1 kg CO2eq for the 7.16 kg/m² reference product). C1 = 0. C3 = fleece/coating incineration with no Module D credit. Scenario 0 (Recycling) = 95% recycled + 5% landfill; Scenario 1 (Reuse) = 80% reused + 20% recycling.';
